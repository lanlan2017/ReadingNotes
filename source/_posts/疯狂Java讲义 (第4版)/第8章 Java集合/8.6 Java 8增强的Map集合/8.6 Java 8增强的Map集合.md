...
`key`和`value`之间存在单向一对一关系,即通过指定的`key`,总能找到唯一的、确定的`value`。从`Map`中取出数据时,只要给出指定的`key`,就可以取出对应的`value`。如果把`Map`的两组值拆开来看,`Map`里的数据有如图8.7所示的结构。
![这里有一张图片](https://image-1257720033.cos.ap-shanghai.myqcloud.com/blog/readbooknote/fangkuangJavaJiangYi3/8/5.png)
从图8.7中可以看出,如果把`Map`里的所有`key`放在一起来看,它们就组成了一个`Set`集合(所有的`key`没有顺序,`key`与`key`之间不能重复),实际上`Map`确实包含了一个`keySet()`方法,用于返回`Map`里所有`key`组成的`Set`集合。
...
**提示**
`Set`与`Map`之间的关系非常密切。虽然`Map`中放的元素是`key-value`对,`Set`集合中放的元素是单个对象,但如果把`key-value`对中的`value`当成`key`的附庸:`key`在哪里, `value`就跟在哪里。这样就可以像对待`Set`一样来对待`Map`了。事实上,`Map`提供了一个`Entry`内部类来封装`key-value`对,而计算`Entry`存储时则只考虑`Entry`封装的`key`。从`Java`源码来看,`Java`是先实现了`Map`,然后**通过包装一个所有`value`都为`null`的`Map`就实现了`Set`集合**。
